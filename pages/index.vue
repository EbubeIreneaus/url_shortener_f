...
const shortenLink = async (e) => {
    ...
    if (data.value.status == 'success') {
        ...
        shortenedlink.qr_src = data.value.qr_src
        shortenedlink.link = data.value.shortened_link
    }
    if (error.value) {
        alert('could not perform this request at the moment!! \n status: server error')
    }
...
<template>
    <div class="py-10 bg-slate-200 text-black/60">
        <div class="w-full md:w-[70%] mx-auto grid content-center min-h-[calc(100vh-70%)] p-[20px] md:p-5 overflow-hidden">
            <div class="text-center">
                ...
                <div class="flex lg:flex-row flex-col gap-1 form">
                    <input type="url" class="flex-grow py-3 ps-3 outline-none border" v-model="uglyLink" id="" placeholder="Enter Link Here">
                    <button type="button" 
                    class="w-fit px-10 bg-slate-950 text-white hover:bg-slate-900 py-3 group" 
                    @click="shortenLink($event)"><i class="fa fa-spinner animate-spin !hidden group-disabled:!inline-block"></i> Shorten URL</button>
                </div> <br>

                <div v-if="shortenedlink.display" class="flex lg:flex-row flex-col items-center 
                justify-between border result bg-white/80 py-3">
                    <a :href="shortenedlink.link" target="_blank" :title="shortenedlink.link" class="px-3 py-2">
                    {{ shortenedlink.link }}
                    </a>
                ...
                <div class="rounded-lg p-[50px] mt-20 shadow-md shadow-black/50 text-start ">
